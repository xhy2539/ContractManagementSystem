...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function initializePage() {
        ...
        // 添加双击事件监听器到所有待选人员列表
        if (availableFinalizerUsersSelect) {
            availableFinalizerUsersSelect.addEventListener('dblclick', () => moveOptions(availableFinalizerUsersSelect, assignedFinalizerUsersSelect));
        }
        if (availableCountersignUsersSelect) {
            availableCountersignUsersSelect.addEventListener('dblclick', () => moveOptions(availableCountersignUsersSelect, assignedCountersignUsersSelect));
        }
        if (availableApprovalUsersSelect) {
            availableApprovalUsersSelect.addEventListener('dblclick', () => moveOptions(availableApprovalUsersSelect, assignedApprovalUsersSelect));
        }
        if (availableSigningUsersSelect) {
            availableSigningUsersSelect.addEventListener('dblclick', () => moveOptions(availableSigningUsersSelect, assignedSigningUsersSelect));
        }

        // 添加双击事件监听器到已选人员列表（支持双击移除）
        if (assignedFinalizerUsersSelect) {
            assignedFinalizerUsersSelect.addEventListener('dblclick', () => moveOptions(assignedFinalizerUsersSelect, availableFinalizerUsersSelect));
        }
        if (assignedCountersignUsersSelect) {
            assignedCountersignUsersSelect.addEventListener('dblclick', () => moveOptions(assignedCountersignUsersSelect, availableCountersignUsersSelect));
        }
        if (assignedApprovalUsersSelect) {
            assignedApprovalUsersSelect.addEventListener('dblclick', () => moveOptions(assignedApprovalUsersSelect, availableApprovalUsersSelect));
        }
        if (assignedSigningUsersSelect) {
            assignedSigningUsersSelect.addEventListener('dblclick', () => moveOptions(assignedSigningUsersSelect, availableSigningUsersSelect));
        }
