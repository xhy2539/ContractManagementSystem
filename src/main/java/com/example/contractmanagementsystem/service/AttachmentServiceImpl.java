...
public class AttachmentServiceImpl implements AttachmentService {
    ...
    public void uploadChunk(String uploadId, int chunkNumber, int totalChunks, MultipartFile fileChunk, String username) throws IOException {
        ...
        if (chunkNumber < 0 || chunkNumber >= totalChunks) {
            logger.error("用户 '{}' 上传无效分块编号: uploadId={}, 分块={}, 总块数={}", username, uploadId, chunkNumber, totalChunks);
            throw new BusinessLogicException("无效的分块编号: " + chunkNumber + "，总块数: " + totalChunks);
        }

        // 检查分块是否已经上传
        if (progress.getUploadedChunks().contains(chunkNumber)) {
            logger.warn("用户 '{}' 尝试重复上传分块: uploadId={}, 分块={}", username, uploadId, chunkNumber);
            // 不抛出异常，而是返回成功（幂等性）
            return;
        }
        ...
        try {
            // 验证分块大小
            long chunkSize = fileChunk.getSize();
            if (chunkSize == 0) {
                logger.error("用户 '{}' 上传空分块: uploadId={}, 分块={}", username, uploadId, chunkNumber);
                throw new BusinessLogicException("分块 " + chunkNumber + " 为空，无法上传");
            }
            ...
            // 验证保存的文件大小
            long savedSize = Files.size(chunkFile);
            if (savedSize != chunkSize) {
                logger.error("用户 '{}' 分块保存失败，大小不匹配: uploadId={}, 分块={}, 期望大小={}, 实际大小={}", 
                            username, uploadId, chunkNumber, chunkSize, savedSize);
                Files.deleteIfExists(chunkFile);
                throw new IOException("分块保存失败：文件大小不匹配");
            }
            ...
            logger.info("用户 '{}' 上传分块成功: uploadId={}, 文件='{}', 分块 {}/{}, 大小={} bytes", 
                       username, uploadId, progress.getOriginalFileName(), chunkNumber + 1, progress.getTotalChunks(), chunkSize);
        } catch (IOException e) {
            ...
            try {
                ...
                logger.warn("清理失败的分块文件时出错: {}", cleanupEx.getMessage());
            }
    ...
    @Override
    @Transactional
    public void cleanupUpload(String uploadId, String username) throws ResourceNotFoundException, AccessDeniedException, IOException {
        FileUploadProgress progress = fileUploadProgressRepository.findById(uploadId)
                .orElseThrow(() -> new ResourceNotFoundException("上传ID '" + uploadId + "' 未找到。"));

        if (!progress.getUploaderUsername().equals(username)) {
            logger.warn("用户 '{}' 尝试清理不属于自己的上传记录 (ID: {}, 属于: {})。", username, uploadId, progress.getUploaderUsername());
            throw new AccessDeniedException("您无权清理此上传 (ID: " + uploadId + ")。");
        }

        logger.info("用户 '{}' 开始清理上传会话: uploadId={}, 原始文件名='{}'", username, uploadId, progress.getOriginalFileName());
        ...
        if (Files.exists(chunkDir) && Files.isDirectory(chunkDir)) {
            try (Stream<Path> walk = Files.walk(chunkDir)) {
                ...
                logger.info("已清理上传ID '{}' 的临时分块目录: {}", uploadId, chunkDir);
            } catch (IOException e) {
                logger.warn("遍历临时分块目录 {} 失败: {}", chunkDir, e.getMessage());
                throw new IOException("清理临时分块目录失败: " + e.getMessage(), e);
            }
        } else {
            logger.info("上传ID '{}' 的临时分块目录不存在或不是目录: {}", uploadId, chunkDir);
        }

        // 2. 删除数据库记录
        try {
            fileUploadProgressRepository.delete(progress);
            logger.info("用户 '{}' 已清理上传会话: uploadId={}, 原始文件名='{}'", username, uploadId, progress.getOriginalFileName());
            auditLogService.logAction(username, "ATTACHMENT_UPLOAD_CLEANED", "清理上传会话: " + progress.getOriginalFileName() + ", uploadId: " + uploadId);
        } catch (Exception e) {
            logger.error("删除上传记录失败 (uploadId: {}): {}", uploadId, e.getMessage(), e);
            throw new IOException("删除上传记录失败: " + e.getMessage(), e);
        }
    }
} 